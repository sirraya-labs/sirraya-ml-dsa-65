...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if pk1 == pk2 && sk1 == sk2 {
        ...
        println!("   PK1 (first 64 chars): {}", encode(&pk1[..32.min(pk1.len())]));
        println!("   PK2 (first 64 chars): {}", encode(&pk2[..32.min(pk2.len())]));
    } else {
    ...
    let valid_with_extracted = Dilithium5::verify(&extracted_pk, b"Test with extracted PK", &sig_with_extracted)?;
    println!("   Result: {}", if valid_with_extracted { "✓ Valid" } else { "✗ Invalid" });
    ...
    println!("   Signature 1 (first 32 chars): {}", encode(&sig1[..16.min(sig1.len())]));
    println!("   Signature 2 (first 32 chars): {}", encode(&sig2[..16.min(sig2.len())]));
    ...
    std::fs::write(format!("{}/secret_key_first_256.hex", output_dir), encode(&sk[..256.min(sk.len())]))?;
...
"#;
    ...
    std::fs::write(format!("{}/verify_signature.rs", output_dir), verification_script)?;
    ...
    println!("• Key generation: {} byte PK, {} byte SK", PUBLICKEYBYTES, SECRETKEYBYTES);
