...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let vc_json = match fs::read_to_string(vc_path) {
        ...
        Err(e) => {
            ...
            eprintln!("        Usage: cargo run --example verify_vc --features w3c -- <credential.json>");
            return Err(e.into());
        }
    };
    ...
    println!("  Issuance:    {}", vc["issuanceDate"].as_str().unwrap_or("unknown"));
    ...
    println!("  Purpose:     {}", proof["proofPurpose"].as_str().unwrap_or(""));
            ...
            match MlDsa65::sign(&test_sk, test_msg) {
                Ok(test_sig) => {
                    ...
                    match MlDsa65::verify(&test_pk, test_msg, &test_sig) {
                        Ok(true) => println!("  [OK] Self-test PASSED - Implementation works correctly"),
                        Ok(false) => println!("  [FAIL] Self-test FAILED - Verification returned false"),
...
fn extract_public_key_from_did(did_key: &str) -> Result<[u8; PUBLICKEYBYTES], Box<dyn std::error::Error>> {
...
fn decode_base64url(input: &str) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    ...
    let cleaned: String = data.trim().chars()
