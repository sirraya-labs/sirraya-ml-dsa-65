...
impl EnterpriseLogger {
    fn new() -> Self {
        ...
        Self {
            log_path: format!("{}/{}_{}.log", REPORT_DIR, LOG_FILE.trim_end_matches(".log"), timestamp),
            start_time: Instant::now(),
        }
    }
    ...
    fn info(&self, message: &str) { self.log("INFO", message); }
    fn warn(&self, message: &str) { self.log("WARN", message); }
    fn error(&self, message: &str) { self.log("ERROR", message); }
    fn success(&self, message: &str) { self.log("PASS", message); }
...
fn get_iso_timestamp() -> String {
    ...
    format!("2026-01-{:02}T{:02}:{:02}:{:02}Z", 
        (days % 31) + 1, hours, mins, secs)
}
...
impl MemoryPeakTracker {
    ...
    fn update_peak(&self) {
        if let Some(current) = get_current_memory_usage() {
            ...
            let _ = self.peak_bytes.fetch_update(Ordering::Relaxed, Ordering::Relaxed, |old| {
...
fn test_tamper_detection(logger: &EnterpriseLogger) -> f64 {
    ...
    logger.success(&format!("Tamper detection rate: {:.1}% ({}/{})", 
        detection_rate * 100.0, tests_passed, total_tests));
...
fn verify_parameters_table1(logger: &EnterpriseLogger) -> ParameterVerification {
    ...
    if K != 6 { mismatches.push(format!("K={}, expected 6", K)); }
    if L != 5 { mismatches.push(format!("L={}, expected 5", L)); }
    if N != 256 { mismatches.push(format!("N={}, expected 256", N)); }
    if Q != 8380417 { mismatches.push(format!("Q={}, expected 8380417", Q)); }
    if GAMMA1 != 524288 { mismatches.push(format!("GAMMA1={}, expected 524288", GAMMA1)); }
    if GAMMA2 != 261888 { mismatches.push(format!("GAMMA2={}, expected 261888", GAMMA2)); }
    if BETA != 98 { mismatches.push(format!("BETA={}, expected 98", BETA)); }
    if TAU != 49 { mismatches.push(format!("TAU={}, expected 49", TAU)); }
    if OMEGA != 55 { mismatches.push(format!("OMEGA={}, expected 55", OMEGA)); }
    if LAMBDA != 192 { mismatches.push(format!("LAMBDA={}, expected 192", LAMBDA)); }
    ...
    ParameterVerification {
        k: K, l: L, n: N, q: Q,
        gamma1: GAMMA1, gamma2: GAMMA2, beta: BETA, tau: TAU,
...
fn benchmark_performance(logger: &EnterpriseLogger) -> ValidationMetrics {
    ...
    logger.info(&format!("Running {} key generation iterations (parallelized)...", KEY_GENERATION_ITERATIONS));
    ...
    let results: Vec<_> = (0..KEY_GENERATION_ITERATIONS)
        ...
        .map(|_| {
            ...
            (keygen_duration, sign_duration, verify_duration, pk, sig, verify_result)
        })
        .collect();
    ...
    logger.success(&format!("Keygen: {:.2}ms ± {:.2}ms (p95: {:.2}ms, p99: {:.2}ms)", 
        metrics.keygen_mean_ms, metrics.keygen_std_ms, 
        metrics.keygen_p95_ms, metrics.keygen_p99_ms));
    logger.success(&format!("Sign:   {:.2}ms ± {:.2}ms (p95: {:.2}ms, p99: {:.2}ms)", 
        metrics.sign_mean_ms, metrics.sign_std_ms,
        metrics.sign_p95_ms, metrics.sign_p99_ms));
    logger.success(&format!("Verify: {:.2}ms ± {:.2}ms (p95: {:.2}ms, p99: {:.2}ms)", 
        metrics.verify_mean_ms, metrics.verify_std_ms,
        metrics.verify_p95_ms, metrics.verify_p99_ms));
...
fn assess_security() -> SecurityAssessment {
    ...
    let delta: f64 = 1.005;
    let blocksize = ((log_q / (2.0 * delta.ln())) * (n as f64).ln()).exp().ceil();
    ...
    SecurityAssessment {
        ...
        status: if blocksize > 500.0 { "SECURE".to_string() } else { "VULNERABLE".to_string() },
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..ENTROPY_SAMPLE_SIZE {
        ...
        if i % 20 == 0 {
            logger.info(&format!("  Entropy sampling: {}/{}", i, ENTROPY_SAMPLE_SIZE));
        }
    }
    ...
    let entropy_analysis = EntropyAnalysis {
        ...
        assessment: if metrics.pk_entropy > 7.8 { "EXCELLENT".to_string() } else { "ADEQUATE".to_string() },
    };
    ...
    logger.success(&format!("Sig Entropy: {:.4} bits/byte", metrics.sig_entropy));
    ...
    let report = EnterpriseValidationReport {
        report_id: format!("SRL-MLDSA65-ENT-{}", SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs()),
        ...
        summary: ValidationSummary {
            total_tests: 7,
            passed_tests: if metrics.all_tests_passed && param_verification.verified { 7 } else { 6 },
            failed_tests: if metrics.all_tests_passed && param_verification.verified { 0 } else { 1 },
            overall_status: if metrics.all_tests_passed && param_verification.verified { "PASSED".to_string() } else { "FAILED".to_string() },
            recommendations: vec![
    ...
    println!("╠══════════════════════════════════════════════════════════════════╣");
    println!("║  Security (λ):     {} bits (NIST Category 3)                      ║", LAMBDA);
    println!("║  Hint bits (ω):    {} bits                                        ║", OMEGA);
    println!("║  Key Generation:  {:>8.2} ms (p95: {:>8.2} ms)                   ║", 
        metrics.keygen_mean_ms, metrics.keygen_p95_ms);
    println!("║  Signing:         {:>8.2} ms (p95: {:>8.2} ms)                   ║", 
        metrics.sign_mean_ms, metrics.sign_p95_ms);
    println!("║  Verification:    {:>8.2} ms (p95: {:>8.2} ms)                   ║", 
        metrics.verify_mean_ms, metrics.verify_p95_ms);
    println!("╠══════════════════════════════════════════════════════════════════╣");
    println!("║  PK Entropy:      {:.4} bits/byte ({:.1}% efficiency)            ║", 
        metrics.pk_entropy, (metrics.pk_entropy / 8.0) * 100.0);
    println!("║  SK Entropy:      {:.4} bits/byte ({:.1}% efficiency)            ║", 
        metrics.sk_entropy, (metrics.sk_entropy / 8.0) * 100.0);
    println!("║  Sig Entropy:     {:.4} bits/byte ({:.1}% efficiency)            ║", 
        metrics.sig_entropy, (metrics.sig_entropy / 8.0) * 100.0);
    println!("╠══════════════════════════════════════════════════════════════════╣");
    println!("║  Key Uniqueness:  {:.1}%                                         ║", 
        metrics.key_uniqueness_rate * 100.0);
    println!("║  Sig Uniqueness:  {:.1}%                                         ║", 
        metrics.signature_uniqueness_rate * 100.0);
    println!("║  Tamper Detect:   {:.1}%                                         ║", 
        metrics.tamper_detection_rate * 100.0);
    println!("╠══════════════════════════════════════════════════════════════════╣");
    println!("║  Table 1:         {}                                    ║", 
        if param_verification.verified { "✓ VERIFIED" } else { "✗ MISMATCH" });
    println!("║  FIPS 204:        {}                                    ║", 
        if metrics.fips_204_compliant { "✓ COMPLIANT" } else { "✗ NON-COMPLIANT" });
    println!("║  All Tests:       {}                                    ║", 
        if metrics.all_tests_passed { "✓ PASSED" } else { "✗ FAILED" });
    println!("╚══════════════════════════════════════════════════════════════════╝\n");
