...
fn verify_s1_coefficients(sk: &[u8; SECRETKEYBYTES]) -> Result<(), String> {
    ...
    println!(
        "  └─ s₁:             bytes 128-{}  ({} bytes per polynomial × {} = {} bytes total)",
        s1_offset + L * poly_size - 1,
        poly_size,
        L,
        L * poly_size
    );
    ...
    if out_of_bounds > 0 {
        return Err(format!(
            "Found {} coefficients outside [-2, 2] range!",
            out_of_bounds
        ));
    }
    ...
    println!(
        "    Zeros:  {} ({:.1}%)",
        zeros,
        zeros as f32 / N as f32 * 100.0
    );
    println!(
        "    +1:     {} ({:.1}%)",
        ones,
        ones as f32 / N as f32 * 100.0
    );
    println!(
        "    -1:     {} ({:.1}%)",
        neg_ones,
        neg_ones as f32 / N as f32 * 100.0
    );
...
fn compare_derivation_methods() {
    ...
    for i in 0..8 {
        if fips_exp[i] != old_exp[i] {
            println!(
                "    Byte {}: {:02x} vs {:02x} ✗ DIFFERENT",
                i, fips_exp[i], old_exp[i]
            );
        } else {
            println!(
                "    Byte {}: {:02x} vs {:02x} ✓ SAME (coincidence)",
                i, fips_exp[i], old_exp[i]
            );
        }
    }
