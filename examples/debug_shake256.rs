...
fn main() {
    ...
    println!("  Signature prefix: {:02x}{:02x}{:02x}...", sig[0], sig[1], sig[2]);
    println!();
    
    // Verify
    let valid = Dilithium5::verify(&pk, msg, &sig).expect("Verification failed");
    println!("✓ Verification: {}", if valid { "SUCCESS" } else { "FAILED" });
    println!();
    
    // Test tampered message
    let tampered = b"Tampered message";
    let should_be_false = Dilithium5::verify(&pk, tampered, &sig).expect("Verification failed");
    println!("✓ Tampered message rejection: {}", if !should_be_false { "SUCCESS" } else { "FAILED" });
    println!();
