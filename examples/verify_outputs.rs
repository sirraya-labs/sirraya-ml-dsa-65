...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Credential ID: {}", vc["id"].as_str().unwrap_or("missing"));
    println!("  Issuer: {}", vc["issuer"].as_str().unwrap_or("missing"));
    println!("  Type: {:?}", vc["type"]);
    println!("  Subject: {}", vc["credential_subject"]["name"].as_str().unwrap_or("missing"));
    println!("  Degree: {}", vc["credential_subject"]["degree"]["type"].as_str().unwrap_or("missing"));
    println!();
    ...
    if valid {
        ...
        println!("    - Issued by: {}", vc["issuer"].as_str().unwrap_or("missing"));
        println!("    - To: {}", vc["credential_subject"]["name"].as_str().unwrap_or("missing"));
        println!("    - For: {}", vc["credential_subject"]["degree"]["type"].as_str().unwrap_or("missing"));
        println!("    - From: {}", vc["credential_subject"]["degree"]["university"].as_str().unwrap_or("missing"));
        println!();
    ...
    println!("✓ Advanced DID Document loaded ({} bytes)", advanced_json.len());
    println!("  DID: {}", advanced_doc["id"].as_str().unwrap_or("missing"));
    ...
    for service in services {
        println!("    {} - {}", service["type"].as_str().unwrap_or("missing"), 
    ...
    let did_multibase = verification_method["public_key_multibase"].as_str().unwrap_or("");
    ...
    println!("  DID Document public key prefix: {}...", &did_pubkey_base64[..32]);
    println!("  Actual public key prefix:        {}...", &actual_base64[..32]);
