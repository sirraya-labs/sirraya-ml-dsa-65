...
pub fn power2round(r: i32) -> (i32, i32) {
    ...
    if r0 > half { r0 -= two_d; }                 // map (4096, 8191] → (−4096, −1]
...
pub fn decompose(r: i32) -> (i32, i32) {
    ...
    if r0 > GAMMA2 { r0 -= alpha; }               // map (GAMMA2, alpha−1] → (−GAMMA2, −1]
...
pub fn make_hint_coeff(z: i32, r: i32) -> i32 {
    ...
    if r1 != v1 { 1 } else { 0 }                  // Alg 39 line 3
}
...
pub fn use_hint_coeff(h: i32, r: i32) -> i32 {
    ...
    if h == 1 {
        if r0 > 0 { (r1 + 1).rem_euclid(m) }      // Alg 40 line 3
        else       { (r1 - 1).rem_euclid(m) }      // Alg 40 line 4
    } else {
        r1                                          // Alg 40 line 5
    }
}
...
pub fn ntt(w: &mut [i32; N]) {
    ...
    while len >= 1 {                               // Alg 41 line 6
        let mut start: usize = 0;                  // Alg 41 line 7
        while start < N {                          // Alg 41 line 8
            m += 1;                                // Alg 41 line 9
            let z = ZETAS[m] as i64;               // Alg 41 line 10
            for j in start..start + len {          // Alg 41 line 11
                // Alg 41 lines 12-14
...
impl Poly {
    ...
    pub fn invntt(&mut self) { invntt(&mut self.coeffs); }
    ...
    pub fn add(&self, rhs: &Self) -> Self {
        let mut r = Self::zero();
        for i in 0..N { r.coeffs[i] = (self.coeffs[i] + rhs.coeffs[i]).rem_euclid(Q); }
        r
    }
    ...
    pub fn reduce(&mut self) {
        for c in self.coeffs.iter_mut() { *c = freeze(*c); }
    }
    ...
    pub fn coeff(&self, i: usize) -> i32 { self.coeffs[i] }
    pub fn set_coeff(&mut self, i: usize, v: i32) { self.coeffs[i] = v; }
    ...
    pub fn power2round(&self) -> (Self, Self) {
        let mut hi = Self::zero(); let mut lo = Self::zero();
        for i in 0..N {
            let (h, l) = power2round(self.coeffs[i]);
            hi.coeffs[i] = h; lo.coeffs[i] = l;
        }
        (hi, lo)
    }
...
pub fn shake128(output: &mut [u8], input: &[u8]) {
    let mut h = Shake128::default(); h.update(input); h.finalize_xof().read(output);
}
pub fn shake256(output: &mut [u8], input: &[u8]) {
    let mut h = Shake256::default(); h.update(input); h.finalize_xof().read(output);
}
pub fn shake256_2(output: &mut [u8], a: &[u8], b: &[u8]) {
    let mut h = Shake256::default(); h.update(a); h.update(b); h.finalize_xof().read(output);
}
...
pub fn rej_ntt_poly(rho: &[u8; SEEDBYTES], col: u8, row: u8) -> Poly {
    ...
    while ctr < N {
        ...
        let z = ((b2_prime as i32) << 16)
              | ((buf[1] as i32) << 8)
              | (buf[0] as i32);
        if z < Q {                                 // Alg 14 line 6
            poly.coeffs[ctr] = z;
            ctr += 1;
        }
    }
...
pub fn expand_mask_poly(rho_prime: &[u8; RHO_PRIME_BYTES], nonce: u16) -> Poly {
    ...
    for i in 0..(N / 2) {
        ...
        let v0 = (buf[b]   as u32)
               | ((buf[b+1] as u32) <<  8)
               | ((buf[b+2] as u32 & 0x0F) << 16);
        let v1 = ((buf[b+2] as u32) >> 4)
               | ((buf[b+3] as u32) <<  4)
               | ((buf[b+4] as u32) << 12);
...
pub fn sample_in_ball(c_tilde: &[u8]) -> Poly {
    ...
    let mut signs: u64 = 0;
    for i in 0..8 { signs |= (sign_bytes[i] as u64) << (8 * i); }

    // Alg 29 lines 6-13: Fisher-Yates for TAU positions
    let mut jbuf = [0u8; 1];
    for i in (N - TAU)..N {                        // Alg 29 line 6
...
pub fn polyt1_pack(buf: &mut [u8; POLYT1_PACKEDBYTES], p: &Poly) {
    for i in 0..(N / 4) {
        let t = [p.coeffs[4*i] as u32 & 0x3FF,
...
pub fn polyt0_pack(buf: &mut [u8; POLYT0_PACKEDBYTES], p: &Poly) {
    ...
    for i in 0..(N / 8) {
        ...
        let t: [u32; 8] = core::array::from_fn(|j| {
            (half - p.coeffs[8*i+j]) as u32 & ((1 << D) - 1)
        });
        let b = i * 13;
        buf[b]    =  t[0]                         as u8;
...
pub fn polyt0_unpack(buf: &[u8; POLYT0_PACKEDBYTES]) -> Poly {
    ...
    for i in 0..(N / 8) {
        ...
        let (b0,b1,b2,b3,b4,b5,b6,b7,b8,b9,b10,b11,b12) = (
            buf[b] as i32, buf[b+1] as i32, buf[b+2] as i32,
            buf[b+3] as i32, buf[b+4] as i32, buf[b+5] as i32,
            buf[b+6] as i32, buf[b+7] as i32, buf[b+8] as i32,
            buf[b+9] as i32, buf[b+10] as i32, buf[b+11] as i32, buf[b+12] as i32,
        );
        let t = [
            b0 | (b1 << 8),
            (b1 >> 5) | (b2 << 3) | (b3 << 11),
        ...
        for j in 0..8 { p.coeffs[8*i+j] = half - (t[j] & mask); }
    }
    p
}
...
pub fn hint_pack(h: &[Poly; K], buf: &mut [u8; OMEGA + K]) -> Option<usize> {
    ...
    for i in 0..K {                                // Alg 20 line 3
        for j in 0..N {                            // Alg 20 line 4
            if h[i].coeffs[j] != 0 {              // Alg 20 line 5
                if index >= OMEGA { return None; }
    ...
    for idx in index..OMEGA { buf[idx] = 0; }
    Some(index)
}
...
pub fn hint_unpack(buf: &[u8; OMEGA + K]) -> Option<[Poly; K]> {
    ...
    for i in 0..K {                                // Alg 21 line 3
        let end = buf[OMEGA + i] as usize;
        if end < index || end > OMEGA { return None; } // Alg 21 line 4
        let first = index;
        while index < end {                        // Alg 21 line 7
    ...
    for i in index..OMEGA {                        // Alg 21 line 16
        if buf[i] != 0 { return None; }           // Alg 21 line 17
    }
    Some(h)
}