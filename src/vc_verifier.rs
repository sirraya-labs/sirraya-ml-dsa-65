...
pub fn extract_public_key_from_did_key(
    did_key: &str,
    ...
    if codec != MLDSA65_MULTICODEC {
        return Err(format!(
            "Expected ML-DSA-65 multicodec (0x{:04x}), got 0x{:04x}",
            MLDSA65_MULTICODEC, codec
        )
        .into());
    }
...
pub fn verify_vc(vc_json: &str) -> Result<bool, Box<dyn std::error::Error>> {
    ...
    // Extract proof
    let proof = vc.get("proof").ok_or("No proof found in credential")?;
